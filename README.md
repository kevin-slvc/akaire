...
辞書の最初の種は `data-rv-dict` に `|` 区切りで渡せる。

```html
<script src="rv-layer.js" data-rv-dict="太文字にして|文字色つけて|無くして"></script>
```

各項目の前後の空白・空項目を除き、重複は先勝ち、20文字超や改行入りを除いて6件まで使う。
指定がなければ空から始まる。保存先がない場合（壊れたJSONを含む）だけ種を読む。
＋か×で変更した後は、空の辞書でも保存内容を優先し、属性の既定語は復活しない。
...
入力欄の直上に、よく使う短い指示を登録できる。チップをクリックすると入力欄の末尾へ
挿入される（既に文があれば改行）。続けて編集でき、クリックだけでは保存しない。

- **登録**: 入力欄に文を書いて「＋ 定型句」（登録後は「＋」）を押す。前後の空白を除き、
  1行・20文字まで・重複なしで最大6件。入力欄の文は残る。
- **削除**: チップにマウスを乗せ、右上の×を押すと即座に外れる。タッチ環境では×を常時表示する。
- **Ctrl+1〜6**: コメント欄が開いている間、その番号の文を挿入して保存する。MacもCtrlを使う。
  Ctrlを押している間だけ番号が出る。

登録順に3列・最大2行で並ぶ。3件以下は1行。入り切らないときは横スクロールでき、＋は右側に残る。
辞書は同じオリジンのページで共通の `rv-layer:dict` に保存され、持ち越しファイルには含まれない。
