...
**1分の動画で見る**: [docs/demo.mp4](docs/demo.mp4)（選ぶ→書く→枠→コピー→済みへ落ちる、までひと通り。
音声なし・字幕あり）。撮り直すときは [tools/record-demo.mjs](tools/record-demo.mjs) を走らせる。
...
そのブラウザで初めてレビュー層を出した人にだけ、左下に4ステップの案内が出る。
選ぶ・書く・枠・コピーを実際にやると次へ進む形で、読むだけの説明ではない。
