...
2. 自動テストは無い。[`tests/manual-checklist.md`](tests/manual-checklist.md) を実機で通し、
   **どの項目を確認したか**をPRに書く
3. 「黙って壊れる」経路（エラーを出さずにデータが失われる・取り違えられる）を直すものは
   優先して見る。その場合、修正前に壊れることを再現した手順も書いてほしい
4. `rv-layer.js` を変えたら `RV_VERSION`（ファイル冒頭のコメントと変数の両方）と
   `CHANGELOG.md` を更新する
